...
This port is read from an environment variable (PORT), not hard-coded.

Using an environment variable makes the application:
...
On the host machine, we also map port 5000 → 5000
...
Exposes the app on the host machine
...
Binds Gunicorn to the port provided via environment variable
...
Uses a Docker Hub access token (Read & Write)
...
manikantamaddy/eme-task:12
manikantamaddy/eme-task:latest
...
Add unit tests and test stage in CI

Add .env.example for developers

Add multi-stage Docker builds

Add CD pipeline (deploy to EC2 or Kubernetes)
